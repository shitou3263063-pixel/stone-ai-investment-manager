...
def test_cn_workflow_runs_report_after_test_failure_and_finalizes_status() -> None:
    workflow = (
        PROJECT_ROOT / ".github" / "workflows" / "daily.yml"
    ...
    assert (
        "- name: Run Stone AI Investment Manager Pro V12.7.1 Final Freeze\n"
        "        id: production\n"
        "        if: always()\n"
        "        continue-on-error: true"
    ) in workflow
...
def test_us_workflow_has_dst_gate_and_independent_artifact() -> None:
    workflow = (
        PROJECT_ROOT / ".github" / "workflows" / "daily-us.yml"
    ).read_text(encoding="utf-8")
    ...
    assert (
        "Inactive seasonal trigger skipped without producing or emailing a report."
        in workflow
    )
    assert "python main.py 2>&1 | tee logs/main.log" in workflow
...
def test_workflow_failure_notification_uses_existing_mail_channel(
    tmp_path: Path,
) -> None:
...
def test_workflow_failure_notification_includes_report_identity(
    tmp_path: Path,
) -> None:
    env_path = tmp_path / ".env"
    env_path.write_text(
    ...
    assert (
        ...
        == "[失败] Stone AI 日报生产未完成 | 美东时间 08:40"
    )
