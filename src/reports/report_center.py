    ...
    *,
    ...
    lines = [
        ...
        *_scenarios(decision),
        ...
        "",
        "## 13. Stone CIO 规则增强分析（OpenAI可选）",
        "",
        f"- 分析模式：{ai.get('mode')}",
        f"- 分析来源：{ai.get('provider')}",
        f"- OpenAI可选复核：{'已参与' if ai.get('openai_participated') else '本次未参与，规则引擎已完成完整分析'}",
        f"- 规则增强总结：{ai.get('summary')}",
        f"- 今日最重要风险：{ai.get('most_important_risk')}",
        f"- 今日最建议做的事：{ai.get('best_action_today')}",
        f"- 今日最不建议做的事：{ai.get('avoid_action_today')}",
