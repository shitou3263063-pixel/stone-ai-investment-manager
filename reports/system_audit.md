...
- 美股/A股/港股/黄金显示0.00：旧市场摘要使用 `market_data.csv` 默认变化值，缺失行情没有区分失败和真实0；V12.1已改为“暂无可靠数据/请求失败/缓存”。
- 双源验证覆盖率：旧路由拿到第一个成功源就返回，导致候选源不足；V12.1已收集 candidates 后再验证。
- 一级来源覆盖率：取决于本次实际成功来源，不再把配置占位算作成功。
- AI状态：SAFE_MODE，原因：missing_openai_key。
- 本周0元、本月金额、债券转权益冲突：旧逻辑把现金预算和未到账债券资金混用；V12.1已拆成确认现金计划和条件性债券到账计划。
- 基础定投无金额：旧逻辑只写继续/暂停；V12.1新增计划日、金额、资金来源和不执行原因。
- 风险评分明细：旧评分来自 MarketAgent 汇总值 78；V12.1已输出八项风险分解。
