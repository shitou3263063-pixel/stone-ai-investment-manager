# Stone AI Investment Manager Pro V12.7.1 Final Freeze

Stone AI Investment Manager 是个人投资管理和投资日报系统。它负责读取资产、获取市场和宏观数据、计算资产偏离、执行风控规则、生成候选建议、调用 AI 做解释复核，并通过 Gmail 发送日报。

V12.7.1 Final Freeze 是在 V12.7.0 Stable 基础上的受控一致性修正：拆分场景DQS与交易权限、集中风险/事件门槛、分离市场吸引力与组合修复优先级、增加持仓白名单，并压缩日报正文。预算、现金安全线、历史交易、资产分类、投资策略、交易规则和模拟网格隔离逻辑保持不变。
...
生产策略配置统一标记为 `config_version: V12.7.1_FINAL_FREEZE`。每类数据只有一个权威来源：
...
- `stable/v12.5`：历史稳定基线，仅用于回退追溯；当前生产版本为V12.7.1 Final Freeze。
...
```bash
git add .
git commit -m "Update Stone AI Investment Manager Pro V12.7.1 Final Freeze"
